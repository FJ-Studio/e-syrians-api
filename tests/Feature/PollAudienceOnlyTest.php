 ...
 * ethnicity, hometown, religion …) is NOT considered sensitive — it
 * describes which groups the poll targets, not which individuals. The
 * mobile/web `AudienceCriteriaSheet` renders the rules next to per-row
 * match / doesn't-match pills, so the `audience` key is exposed to all
 * viewers (guest, audience member, non-member, creator).
 */
